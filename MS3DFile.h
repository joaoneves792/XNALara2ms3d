...
typedef struct
{
    ...
    //These values are only used during the conversion XNA -> ms3d
    float   normal[3];
    float   uv[2];
    short   boneIndicesGlobal[4];
    short   boneIndicesLocal[4];
    float   weights[4];
}XNA_vertex_t;
...
typedef struct
{
    ...
    //These ids are only used when importing XNALara models (ms3d deals only with names)
    int 	    id;
    int             parentId;
...
public:
	...
	bool LoadXNAFromFile(const char* lpszFileName);
